 ...
 */
public class WarehouseManagerController extends ControllerBase<WarehouseManagerModel, WarehouseManagerView> implements ActionListener, MouseListener
{
	...
	public void LoadThuocLenTable(int luachon, String tenthuoc)
	{
        ...
        {
            "Mã thuốc", "Tên thuốc", "Thành phần", "Chỉ định", "Đơn vị", "Còn lại", "Mã nhà cung cấp", "Nhà cung cấp", "Ngày sản xuất", "Ngày hết hạn"
        };
        ...
        if (luachon == 0 )
        {
        	resultSet = model.getData("SELECT T.MATHUOC, "
                    + "T.TENTHUOC, "
                    + "T.THANHPHAN, "
                    + "T.CHIDINH, "
                    + "T.DONVI, "
                    + "T.CONLAI, "
                    + "T.NHACUNGCAP_REF.MANCC, "
                    + "T.NHACUNGCAP_REF.TENNCC, "
                    + "T.NGAYSANXUAT, "
                    + "T.NGAYHETHAN\n" +
                    "FROM THUOC T");
        }
	...
	public void LoadThucAnLenTable(int luachon, String tenloaithucan)
	{
		...
		final String[] _columnsTBKho =
        {
            "Mã thức ăn", "Mã loại thức ăn", "Loại thức ăn", "Còn lại", "Đơn vị","Ngày sản xuất", "Ngày hết hạn", "Mã nhà cung cấp", "Nhà cung cấp"
        };
        view.dtm = new DefaultTableModel(_columnsTBKho,0);
        ...
        if (luachon==0)
        {
        	resultSet = model.getData("select TA.MATHUCAN, "
                    + "TA.LOAITHUCAN_REF.MALOAITHUCAN,"
                    + "TA.LOAITHUCAN_REF.TENLOAITHUCAN, "
                    + "TA.CONLAI, "
                    + "TA.LOAITHUCAN_REF.DONVI, "
                    + "TA.NGAYSANXUAT, "
                    + "TA.NGAYHETHAN, "
                    + "TA.NHACUNGCAP_REF.MANCC, "
                    + "TA.NHACUNGCAP_REF.TENNCC\n" +
                    "from THUCAN TA");
        }
        else
        {
        	resultSet = model.getData("select TA.MATHUCAN, "
        			+ "TA.LOAITHUCAN_REF.MALOAITHUCAN,"
        			+ "TA.LOAITHUCAN_REF.TENLOAITHUCAN, "
        			+ "TA.CONLAI, TA.LOAITHUCAN_REF.DONVI, "
        			+ "TA.NGAYSANXUAT, "
        			+ "TA.NGAYHETHAN, "
        			+ "TA.NHACUNGCAP_REF.MANCC, "
        			+ "TA.NHACUNGCAP_REF.TENNCC  "
	...
	public void CapNhatThuoc(String mathuoc, String tenthuoc, String thanhphan, String chidinh, String donvi, String conlai, String mancc,String datensx, String datenhh)
	{
		try 
		{
			String query = "update thuoc set tenthuoc = '"+tenthuoc+"', "
	    			+ "thanhphan = '"+thanhphan+"', "
					+ "chidinh = '"+chidinh+"', "
					+ "donvi = '"+donvi+"', "
					+ "conlai = "+conlai+", "
					+ "NHACUNGCAP_REF = (select REF(ncc) from nhacungcap ncc where mancc = '"+mancc+"'), "
					+ "ngaysanxuat = TO_DATE('"+datensx+"', 'yyyy-mm-dd'), ngayhethan = TO_DATE('"+datenhh+"', 'yyyy-mm-dd') "
					+ "where mathuoc = '"+mathuoc+"'";
			model.editData(query);
	...
	public void CapNhatThucAn(String mathucan, String mancc, String maloaithucan, String conlai, String datensx, String datenhh)
	{
		try {
			String query = "UPDATE thucan SET nhacungcap_ref = (select REF(ncc) from nhacungcap ncc where ncc.mancc = '"+mancc+"'), "
					+ "thucan.LOAITHUCAN_REF = (select REF(lta) from loaithucan lta where lta.maloaithucan = '"+maloaithucan+"'), "
					+ "conlai = "+conlai+", "
					+ "NGAYSANXUAT = TO_DATE('"+datensx+"', 'yyyy-mm-dd'), "
					+ "ngayhethan = TO_DATE('"+datenhh+"', 'yyyy-mm-dd') "
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        //Radio button
        //Chọn thuốc
        if (view.DRUG_SELECTED_COMMAND.equals(_command))
        {
        ...
        //chọn thức ăn
        if (view.FOOD_SELECTED_COMMAND.equals(_command))
        {
        ...
        //Search button
        if (view.SEARCH_COMMAND.equals(_command))
        {
        ...
        if (view.SELECT_PROVIDER_COMMAND.equals(_command))
        {
            String _maNCC = null;
        ...
        //select food button
        if (view.SELECT_FOOD_COMMAND.equals(_command))
        {
            FoodView fv = new FoodView();
            FoodModel fm = new FoodModel();
            FoodController fc = new FoodController(fm, fv);
        ...
        //add button
        if (view.ADD_COMMAND.equals(_command))
        {
            ...
            if (view._rdbtThuoc.isSelected())
            {
                db.saveAutoId(
                    Entity.idGenner(
                        ...
                        "MATHUOC", 
                        "Thuoc_objtyp", 
                        "Thuoc_objtyp('111', '"+ view._txtfTen.getText() +"','"+ view._txtaThanhPhan.getText() +"','"+ view._txtaChiDinh.getText() +"','"+ view._txtfDonVi.getText() +"', "+ view._txtfConLai.getText() +",null,"
                        + "TO_DATE('"+ datensx +"', 'yyyy-mm-dd'),"
                        + "TO_DATE('"+ datenhh +"', 'yyyy-mm-dd'))",
                        new RefPayload("NhaCungCap", "MaNCC", ""+ WarehouseManagerView._txtfMaNCC.getText() +"", "NhaCungCap_ref")
                    			)
        			);
            ...
            else
            {
            	...
            	ref.add(new RefPayload("NhaCungCap", "MaNCC", ""+ WarehouseManagerView._txtfMaNCC.getText() +"", "NhaCungCap_ref"));
            	ref.add(new RefPayload("LoaiThucAn", "MaLoaiThucAn", ""+ WarehouseManagerView._txtfMaLoaiThucAn.getText() +"", "LoaiThucAn_ref"));
            	db.saveAutoId(
            			Entity.idGenner(
                            "THUCAN", 
                            "MATHUCAN", 
                            "ThucAn_objtyp", 
                            "ThucAn_objtyp('111', null, null, "+ view._txtfConLai.getText() +","
                            + "TO_DATE('"+ datensx +"', 'yyyy-mm-dd'),"
                            + "TO_DATE('"+ datenhh +"', 'yyyy-mm-dd'))",
                            ref
                        			)
        ...
        //remove button
        if (view.REMOVE_COMMAND.equals(_command))
        {
            int row;
            if (view._rdbtThuoc.isSelected())
            {
                ...
                model.editData("DELETE FROM THUOC WHERE MATHUOC = '" + id + "'");
            ...
            else
            {
                ...
                String id = view._tbKho.getModel().getValueAt(row, 0).toString();
                model.editData("DELETE FROM THUCAN WHERE MATHUCAN = '" + id + "'");
                JOptionPane.showMessageDialog(null, "Success");
                LoadThucAnLenTable(0, "");
            }
            return;
        ...
        //update button
        if (view.UPDATE_COMMAND.equals(_command))
        {
            int row = -1;
            String datensx = formatdate.format(view._dateNSX.getDate());
            String datenhh = formatdate.format(view._dateNHH.getDate());
            if (view._rdbtThuoc.isSelected())
            {
                ...
                {
                	CapNhatThuoc(view._txtfMa.getText(), view._txtfTen.getText(), view._txtaThanhPhan.getText(), view._txtaChiDinh.getText(), view._txtfDonVi.getText(), view._txtfConLai.getText(), view._txtfMaNCC.getText(), datensx, datenhh);
            		LoadThuocLenTable(0, "");
                }
            ...
            else
            {
                ...
                {
                	CapNhatThucAn(view._txtfMa.getText(), WarehouseManagerView._txtfMaNCC.getText(), WarehouseManagerView._txtfMaLoaiThucAn.getText(), view._txtfConLai.getText(), datensx, datenhh);
            		LoadThucAnLenTable(0, "");
                }
            }
            return;
        ...
        //export button
        if (view.EXPORT_COMMAND.equals(_command))
        {
        	...
        	for (int row = rows - 1;row>=0;row--)
        	{
            	...
            	if (check.equals("Thuốc"))
            	{
            		try 
            		{
            			String query = "update thuoc set conlai = conlai -"+Float.parseFloat(view._tbXuatKho.getModel().getValueAt(row, 3).toString())+" where mathuoc = '"+view._tbXuatKho.getModel().getValueAt(row, 1)+"'";
                		db.send(query);
            	...
            	else
            	{
            		try 
            		{
            			String query = "update thucan set conlai = conlai -"+Float.parseFloat(view._tbXuatKho.getModel().getValueAt(row, 3).toString())+" where mathucan = '"+view._tbXuatKho.getModel().getValueAt(row, 1)+"'";
                		db.send(query);
        ...
        if (view.TRANSFER_TO_RIGHT_COMMAND.equals(_command))
        {
            ...
            else
            {
            	...
            	if (view._rdbtThuoc.isSelected())
            	{
            		...
            		{
            			view.dtmXK.addRow(new Object[]{ "Thuốc",
            					view._tbKho.getModel().getValueAt(row, 0), 
								view._tbKho.getModel().getValueAt(row, 1),
								_giamconlai,
								view._tbKho.getModel().getValueAt(row, 4), 
								view._tbKho.getModel().getValueAt(row, 7),
								view._tbKho.getModel().getValueAt(row, 8),
            	...
            	else
            	{
            		...
            		if (f>=_giamconlai && _giamconlai!=0)
            		{
            			...
            			view.dtmXK.addRow(new Object[]{ "Thức ăn",
            					view._tbKho.getModel().getValueAt(row, 0), 
								view._tbKho.getModel().getValueAt(row, 2),
								_giamconlai,
								view._tbKho.getModel().getValueAt(row, 4), 
								view._tbKho.getModel().getValueAt(row, 8),
								view._tbKho.getModel().getValueAt(row, 5),
								view._tbKho.getModel().getValueAt(row, 6)
								});
            		}
            		else
            		{
        ...
        if (view.TRANSFER_TO_LEFT_COMMAND.equals(_command))
        {
