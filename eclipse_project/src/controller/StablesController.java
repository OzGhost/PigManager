 ...
 * @author duyphuoc
 */
public class StablesController extends ControllerBase<StablesModel,StablesView> implements MouseListener{
    ...
    if(StablesView.SAVE_COMMAND.equals(command)){
       ...
       //reset row edited on jtable
       view.dtm.setValueAt(view.number_tx.getText().toString(), row, 2);
       view.dtm.setValueAt(view.location_tx.getText().toString(), row, 3);
       view.dtm.setValueAt(view.describe_tx.getText().toString(), row, 4);
...
//select button event
    if(view.SELECT_COMMAND.equals(command)){
      int row = view.gridTb.getSelectedRow();
      if(row ==-1){
          JOptionPane.showMessageDialog(null, "Please select");
          return;
      }
      String var= view.gridTb.getModel().getValueAt(row, 0).toString(); 
