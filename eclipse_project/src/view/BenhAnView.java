...
public class BenhAnView extends ViewBase{
	...
	public BenhAnView(){
        ...
        final TitledBorder titleTimKiem=BorderFactory.createTitledBorder(border,"Tìm kiếm bệnh án");
        
        panelTop=new JPanel();
        panelTop.setBorder(titleTimKiem);
        panelTop.setPreferredSize(new Dimension(400, 70));
        Layer.put(panelTop).in(sl_panelMain).atTop(panelMain).withMargin(10).atLeft(panelMain).withMargin(10).atRight(panelMain).withMargin(10);
        
        cbxTimKiem =new JComboBox<>();
        cbxTimKiem.addItem("Mã bệnh án");
        ...
        Layer.put(panelMid).in(sl_panelMain).atLeft(panelMain).withMargin(10).bottomOf(panelTop).withMargin(10).atRight(panelMain).withMargin(10);

        JLabel lblMaBenhAn = new JLabel("Mã bệnh án");
        Layer.put(lblMaBenhAn).in(sl_panelMid).atLeft(panelMid).withMargin(180).atTop(panelMid).withMargin(15);      
        
        JLabel lblMaSoHeo=new JLabel("Mã số heo");
        ...
        Layer.put(dateNgayTao).in(sl_panelMid).atLeft(panelMid).withMargin(300).atTop(lblNgayTao).withMargin(0).atRight(txtMaBenhAn).withMargin(0);
        
        btnChonHeo =Genner.createButton("Chọn", Genner.MEDIUM_SIZE);
        Layer.put(btnChonHeo).in(sl_panelMid).leftOf(txtMaSoHeo).withMargin(15).atTop(txtMaSoHeo).withMargin(0).atBottom(txtMaSoHeo).withMargin(0);
        ...
        final String[] columnBenhAn={"Mã bệnh án","Mã số heo","Ngày tạo"};
        ...
        JScrollPane scrolltbBenhAn = new JScrollPane(tbBenhAn, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
        ...
        Layer.put(btnThem).in(sl_panelMain).leftOf(panelBot).withMargin(50).bottomOf(panelMid).withMargin(30).atRight(panelMain).withMargin(50);
        btnThem.setActionCommand(INSERT_COMMAND);
        
        btnXoa = Genner.createButton("Xóa", Genner.MEDIUM_SIZE);
        Layer.put(btnXoa).in(sl_panelMain).leftOf(panelBot).withMargin(50).bottomOf(btnThem).withMargin(30).atRight(panelMain).withMargin(50);
        btnXoa.setActionCommand(DELETE_COMMAND);
        
        btnCapNhat = Genner.createButton("Cập nhật", Genner.MEDIUM_SIZE);
        Layer.put(btnCapNhat).in(sl_panelMain).leftOf(panelBot).withMargin(50).bottomOf(btnXoa).withMargin(30).atRight(panelMain).withMargin(50);
        btnCapNhat.setActionCommand(UPDATE_COMMAND);
        
        btnCTBenh = Genner.createButton("Chi tiết", Genner.MEDIUM_SIZE);
        Layer.put(btnCTBenh).in(sl_panelMain).leftOf(panelBot).withMargin(50).bottomOf(btnCapNhat).withMargin(30).atRight(panelMain).withMargin(50);
        btnCTBenh.setActionCommand(DETAIL_COMMAND);
        
        btnDungThuoc = Genner.createButton("Dùng thuốc", Genner.MEDIUM_SIZE);
        Layer.put(btnDungThuoc).in(sl_panelMain).leftOf(panelBot).withMargin(50).bottomOf(btnCTBenh).withMargin(30).atRight(panelMain).withMargin(50);
