...
public class FoodView extends ViewBase
{
	...
	public JButton				_btnTimKiem, _btnThem, _btnXoa, _btnCapNhat, _btnChonLoai;
	...
	public FoodView() {
		...
		JScrollPane _scrollMota = new JScrollPane(_txtaMota, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
		...
		Layer.put(_lblMuaBaoDong).in(sl_panelMid).bottomOf(_lblDonVi).withMargin(30).leftOf(_txtfTenLoai)
				.withMargin(30);
		...
		JScrollPane _scrollTBThucAn = new JScrollPane(_tbThucAn, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
		...
		sl_contentpane.putConstraint(SpringLayout.HORIZONTAL_CENTER, _panelTop, 0, SpringLayout.HORIZONTAL_CENTER,
				contentpane);
		Layer.put(_panelMid).in(sl_contentpane).atLeft(contentpane).withMargin(200).atRight(contentpane).withMargin(200)
				.bottomOf(_panelTop).withMargin(10);
		Layer.put(_panelBot).in(sl_contentpane).atLeft(contentpane).withMargin(200).bottomOf(_panelMid).withMargin(10)
				.atRight(contentpane).withMargin(200).atBottom(contentpane).withMargin(30);
		sl_contentpane.putConstraint(SpringLayout.VERTICAL_CENTER, _btnCapNhat, 0, SpringLayout.VERTICAL_CENTER,
				_panelBot);
		...
		Layer.put(_btnChonLoai).in(sl_contentpane).bottomOf(_btnCapNhat).withMargin(20).leftOf(_panelBot)
				.withMargin(25);
